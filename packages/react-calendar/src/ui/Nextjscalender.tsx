...
import { googleLink, outlookLink } from '../functions/function';
...
export default function NextJsCalendar({
  Events,
  stylesOverrides
}: {
  Events: IeventData[];
  ...
  const MakeGoogleLink = (e: IeventData) => {
    return googleLink({
      title: e.event_title,
  ...
  const MakeOutlookLink = (e: IeventData) => {
    return outlookLink({
      title: e.event_title,
      startTime: e.event_date,
      description: e.event_description
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          ...
          <div className="-mx-1 -mb-1">
            ...
            <div className="flex flex-wrap border-t border-l">
              {blankDays.map((blankday: any, index: React.Key | null | undefined) => (
                <div key={index} style={{ width: '14.28%', height: '120px' }} className="text-center border-r border-b px-4 pt-2"></div>
              ))}
              {noOfDays.map((date: number, dateIndex: number) => (
                <div key={dateIndex} style={{ width: '14.28%', height: '120px' }} className={`${styles.calendarDay}`}>
                  ...
                  <div style={{ height: '80px' }} className="overflow-y-auto mt-1">
                    {events.filter((event: { event_date: string | number | Date; }) => new Date(event.event_date).toDateString() === new Date(year, month, date).toDateString()).map((event: { event_theme: string; event_link: string | undefined; event_title: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | null | undefined; google_event_link: string | undefined; outlook_event_link: string | undefined; }, eventIndex: any) => (
                      <div
                        className={`px-2 py-1 rounded-lg mt-1 overflow-hidden border ${styles.eventCard[event.event_theme as keyof typeof styles.eventCard] || ''
                          }`}
                      >
                        <ContextMenu>
                          <ContextMenuTrigger>
